...
export const pollAllPage = async (
  req: express.Request,
  res: express.Response
) => {
  const Poll = mongoose.model("Poll");
...
export const pollUserPage = async (
  req: express.Request,
  res: express.Response
) => {
...
export const pollEditPage = async (
  req: express.Request,
  res: express.Response
) => {
...
export const pollAdd = async (
  ...
  try {
    ...
    req.flash(
      "success",
      `Poll submitted | <a href="/poll/${hashids.encodeHex(
        poll.id
      )}">View poll</a>`
    );
  } catch (error) {
    req.flash(
      "error",
      `${error.errors.name.message || "Poll submission failed"}`
    );
  }
...
export const pollDelete = async (
  ...
  try {
    ...
    req.flash(
      "error",
      `${error.errors.name.message || "Poll deletion failed"}`
    );
    res.redirect("back");
  }
...
export const pollOnePage = async (
  req: express.Request,
  res: express.Response
) => {
  ...
  if (userVote) {
    ...
    userVote = userVote.votes.filter(vote =>
      vote.person.equals(req.user!._id)
    )[0];
  }
